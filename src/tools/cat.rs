...
pub fn cat_handler(args: CatArgs) -> String {
    args.file_names
        ...
        .map_or(String::with_capacity(0), |file_names| {
            handle_file_names(file_names, &args)
        })
}

fn handle_file_names(file_names: &[String], args: &CatArgs) -> String {
    let estimated_size = file_names
        .iter()
        .map(|name| fs::metadata(name).map(|m| m.len() as usize).unwrap_or(0))
        .sum();
    let mut result = String::with_capacity(estimated_size); // Initialize with estimated capacity
    ...
    file_names
        ...
        .for_each(|file_name| match fs::read_to_string(file_name) {
            Ok(file_content) => {
                result.push_str(&parse_file_contents(file_content, &mut line_count, args))
            }
            Err(_) => result.push_str(&format!("cat: {}: No such file or directory\n", file_name)),
        });

    result
}

fn parse_file_contents(file_content: String, line_count: &mut u128, args: &CatArgs) -> String {
